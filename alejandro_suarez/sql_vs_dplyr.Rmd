...
```{r}
DATA_DIR <- file.path("C:/Users/alejandro.suarez/Desktop/Big_data_management/sql_dplyr_exercise/football_data")
library(RSQLite)
sql_con <- dbConnect(
  SQLite(), 
...
```{sql connection=sql_con}
SELECT AVG(overall_rating) AS mean_overall_rating
, STDEV(finishing) AS finishing_STD
FROM Player_Attributes 
LIMIT 5
```
...
FROM Player_attributes
GROUP BY sprint_speed " %>%
  dbGetQuery(sql_con, .) %>%
  ggplot(., aes(sprint_speed, potential)) +
...
* List 5 players and their highest rating that had the lowest ratting ever.
```{sql connection=sql_con}
...
```{sql connection=sql_con}
SELECT pa.avg_home_goals
, p.team_long_name
, c.name
FROM(
SELECT AVG(home_team_goal) AS avg_home_goals
, country_id
, home_team_api_id
FROM Match
WHERE season = '2008/2009' AND country_id = 15722
GROUP BY home_team_api_id
ORDER BY avg_home_goals DESC) pa
LEFT JOIN team p
ON pa.home_team_api_id = p.team_api_id
LEFT JOIN Country c
ON pa.country_id = c.id
LIMIT 1
```
